...
class WhiteBalanceController:
    ...
    def __init__(self, camera_interface):
        ...
        logger.debug(f"Bit depth: {self.bit_depth}, Target mean: {self.wb_mean_min}-{self.wb_mean_max}")
    ...
    def set_bit_depth(self, bit_depth: BitDepth):
        ...
        logger.info(f"Set bit depth to {bit_depth}, target mean: {self.wb_mean_min}-{self.wb_mean_max}")
    ...
    def find_exposure_for_wb(self) -> Tuple[float, WBResult]:
        ...
        try:
            ...
            bias = self.ADU_BIAS_16BIT if self.bit_depth == BitDepth.DEPTH_16BIT else self.ADU_BIAS_LESS16BIT
            if mean_100ms < bias:
            ...
            while iterations < self.WB_EXP_ITERATIONS_MAX and exposure < self.MAX_EXPOSURE:
                ...
                logger.debug(f"Iteration {iterations}: exposure={exposure:.1f}ms, mean={current_mean:.1f}")
                
                iterations += 1
                
                if self.wb_mean_min <= current_mean <= self.wb_mean_max:
                    logger.info(f"Found optimal exposure: {exposure:.1f}ms (mean: {current_mean:.1f})")
    ...
    def calculate_scales(self) -> Tuple[float, float, float]:
        ...
        logger.info(f"Calculated scales - R: {red_scale:.4f}, G: {green_scale:.4f}, B: {blue_scale:.4f}")
...
def main():
    ...
    if success:
        ...
        print(f"Channel means - R: {wb_controller.r_mean:.1f}, "
              f"G: {wb_controller.g_mean:.1f}, B: {wb_controller.b_mean:.1f}")
        print(f"Scale factors - R: {wb_controller.red_scale:.4f}, "
